...
static Args parse_args(int argc, char** argv) {
    ...
    for (int i=1;i<argc;++i) {
        if (!std::strncmp(argv[i], "--backend=", 10)) a.backend = argv[i]+10;
        else if (!std::strncmp(argv[i], "--nx=", 5)) a.nx = std::atoi(argv[i]+5);
        else if (!std::strncmp(argv[i], "--ny=", 5)) a.ny = std::atoi(argv[i]+5);
        else if (!std::strncmp(argv[i], "--nz=", 5)) a.nz = std::atoi(argv[i]+5);
        else if (!std::strncmp(argv[i], "--reps=", 7)) a.reps = std::atoi(argv[i]+7);
        else if (!std::strcmp(argv[i], "--float32")) a.f32 = true;
        else if (!std::strcmp(argv[i], "--float64")) a.f32 = false;
    }
    return a;
}
...
int main(int argc, char** argv)
{
    ...
    for (int f=0; f<nfields; ++f) {
        ...
        for (std::size_t i=0;i<v.size();++i) v[i] = double(f) + 0.001*double(i + ox + oy*args.nx + oz*args.nx*args.ny);
        ...
        fc.register_scalar(
            ...
            sizeof(double),
            {lx, ly, lz},
            {(std::ptrdiff_t)sizeof(double),
             (std::ptrdiff_t)(sizeof(double)*lx),
    ...
    cfg.precision = args.f32 ? WriterConfig::Precision::Float32
                             : WriterConfig::Precision::Float64;
    ...
    if (args.backend == "xmf" || args.backend == "xdmf") {
        ...
        if (world_rank==0) std::cerr << "Unknown --backend="<< args.backend << " (use xmf|cgns)\n";
        MPI_Finalize();
        return 2;
    }
    ...
    if (world_rank==0) {
        const std::string tag = "io_" + args.backend + "_" +
            std::to_string(args.nx)+"x"+std::to_string(args.ny)+"x"+std::to_string(args.nz) +
            "_p"+std::to_string(world_size);
