...
TEST_CASE("MPI: only rank 0 performs IO; steps synchronized", "[mpi][scheduler]") {
  ...
  mesh::Layout layout; mesh::HaloExchange halos; mesh::Boundary bcs;
  ...
  auto w = std::make_unique<RankAwareWriter>(); auto* W = w.get();
  m.set_writer(std::move(w));

  m.configure_program("noop", {});
  master::TimeControls tc; tc.dt=0.1; tc.t_end=0.2; tc.write_every=1;
  m.run(tc);
  ...
  int sum=0; MPI_Allreduce(&local_steps,&sum,1,MPI_INT,MPI_SUM,MPI_COMM_WORLD);
  REQUIRE(sum == 2*size);
}
