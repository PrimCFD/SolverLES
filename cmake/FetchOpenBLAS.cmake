...
set(OPENBLAS_URL "https://github.com/xianyi/OpenBLAS/archive/refs/tags/v${OPENBLAS_VERSION}.tar.gz"
...
# Makefile build knobs
# - Build SHARED only (NO_STATIC=1) so downstream can reliably link a .so/.dylib/.dll
# - Do NOT build or run tests/benchmarks
# - DYNAMIC_ARCH=1 gives broad CPU coverage on x86_64
set(_ob_make_opts)
list(APPEND _ob_make_opts
  "USE_OPENMP=0"
  "DYNAMIC_ARCH=1"
...
# Build only what we need: libraries + the shared library target. No tests will be invoked.
ExternalProject_Add(openblas_ep
  ...
  INSTALL_COMMAND    ${MAKE_EXECUTABLE} -s ${_ob_make_opts} PREFIX=<INSTALL_DIR> install
  LOG_DOWNLOAD 1
  LOG_UPDATE 1
  LOG_CONFIGURE 1
  LOG_BUILD 1
  ...
  BUILD_BYPRODUCTS   "${_openblas_install}/lib/libopenblas${CMAKE_SHARED_LIBRARY_SUFFIX}"
)

# Imported target: prefer the canonical soname (libopenblas.so / .dylib / .dll)
# We install to <prefix>/lib on Linux/macOS. (Windows Makefile builds are uncommon; adjust if needed.)
set(_ob_lib "${_openblas_install}/lib/libopenblas${CMAKE_SHARED_LIBRARY_SUFFIX}")

# If a distro emits only a fully versioned filename, fall back to first matching .so* / .dylib* / .dll*
...
if(NOT _ob_lib)
  message(FATAL_ERROR "OpenBLAS shared library not found under ${_openblas_install}/lib after build.")
endif()

# Define the imported target (supports .so/.dylib/.dll)
add_library(OpenBLAS::OpenBLAS UNKNOWN IMPORTED GLOBAL)
set_target_properties(OpenBLAS::OpenBLAS PROPERTIES
  IMPORTED_LOCATION             "${_ob_lib}"
  INTERFACE_INCLUDE_DIRECTORIES "${_openblas_install}/include"
)
...
set(OpenBLAS_INCLUDE_DIRS "${_openblas_install}/include" CACHE PATH "" FORCE)
set(OpenBLAS_LIBRARIES    "${_ob_lib}"                    CACHE FILEPATH "" FORCE)
