...
namespace core::master { class FieldCatalog; struct RunContext; }
...
// Bitmask helpers
inline Phase operator|(Phase a, Phase b){ return Phase(unsigned(a)|unsigned(b)); }
inline bool  has(Phase mask, Phase bit){ return (unsigned(mask)&unsigned(bit))!=0; }
...
struct IAction {
  ...
  virtual void execute(const MeshTileView& tile,
                       FieldCatalog& fields,
                       double dt) = 0;
};
