...
template <class T>
bool exchange_ghosts(Field<T>& f, const Mesh& m, MPI_Comm comm, int tag_base = 200,
                     core::master::Stagger stagger = core::master::Stagger::Cell)
{
    ...
    if ((cper[0] != (m.periodic[0] ? 1 : 0)) || (cper[1] != (m.periodic[1] ? 1 : 0)) ||
        (cper[2] != (m.periodic[2] ? 1 : 0)))
        return false;
    ...
    for (int idx = 0; idx < 26; ++idx)
    {
        ...
        if (d.peer != MPI_PROC_NULL && d.peer != myrank)
        {
            ...
            MPI_Request r{};
            MPI_Irecv(rbuf[idx].data(), static_cast<int>(d.count), Tmpi, d.peer, d.tag_recv, comm,
                      &r);
            reqs.push_back(r);
        }
    }
    ...
    for (int idx = 0; idx < 26; ++idx)
    {
        ...
        else
        {
            pack(d, sbuf[idx]);
            MPI_Request s{};
            MPI_Isend(sbuf[idx].data(), static_cast<int>(d.count), Tmpi, d.peer, d.tag_send, comm,
                      &s);
            reqs.push_back(s);
        }
    }
