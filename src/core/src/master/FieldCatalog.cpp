...
void FieldCatalog::register_scalar(std::string name,
                                   void* host_ptr,
                                   std::size_t elem_size,
                                   std::array<int,3> extents,
                                   std::array<std::ptrdiff_t,3> strides)
{
  if (idx_.count(name)) throw std::runtime_error("Field already registered: " + name);
  AnyFieldView v;
  v.name = name;
  v.host_ptr = host_ptr;
...
AnyFieldView FieldCatalog::view(std::string_view name) const {
  ...
  if (it == idx_.end()) throw std::runtime_error("Unknown field: " + std::string(name));
