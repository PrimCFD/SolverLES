...
namespace master
{
    ...
    Scheduler::Scheduler(const RunContext& rc, const mesh::Layout& layout,
                         mesh::HaloExchange& halos, mesh::Boundary& bcs, FieldCatalog& fields,
                         io::IWriter& writer)
    ...
    void Scheduler::run(const TimeControls& tc)
    {
        ...
        for (int s = 0; s < steps; ++s, t += tc.dt)
        {
            ...
            if (tc.write_every > 0 && (s % tc.write_every == 0))
            {
                ...
                req.step = s;
                req.time = t;
                for (auto& v : fields_.selected_for_output())
